      ...
      <form onSubmit={handleSubmit} className="assignment-form">
       ...
       <h3>Sub-Assignments</h3>
          ...
          <div key={idx} className="sub-assignment-card">
            ...
            {sub.isDynamic && (
              <div className="dynamic-questions">
                  ...
                  <div key={qIdx} className="question-card">
                    ...
                    <div className="form-group">
                      ...
                      <input
                        ...
                        value={q.answer}
                        onChange={(e) => handleDynamicQuestionChange(idx, qIdx, "answer", e.target.value)}
                        required
                      />
                    </div>

                    {sub.dynamicQuestions.length > 1 && (
                      <Button onClick={() => removeDynamicQuestion(idx, qIdx)} variant="danger" className="remove-question-btn">
                        Remove Question
                      </Button>
                    )}
                  </div>
                ...
                <Button onClick={() => addDynamicQuestion(idx)} variant="primary" className="add-question-btn">
        ...
        <div className="form-actions">
          <Button onClick={addSubAssignment} variant="primary" className="add-sub-btn">
            + Add Sub-Assignment
          </Button>
...
const StudentSummaryView = ({ summary, onBack, loading, error }) => {
  ...
  const renderSubmission = (enteredValues, answerKey, correctCount, wrongCount, progressPercent) => {
    if (!enteredValues) return <p className="no-data-text">No answers submitted</p>;
...
const StudentsModal = ({ module, studentSummaryProps }) => {
  ...
  const StudentListItem = ({ student, onClick }) => (
    <li className="student-list-item" onClick={onClick}>
...
const AssignmentCard = ({ assignment, onDeleteModule, onDeleteSubAssignment, onOpenStudentsModal, onEditModule, onEditSubAssignment, deleting }) => {
  const { _id: moduleId, moduleName, assignedDate, subAssignments, assignmentPdf, category } = assignment;
    ...
    <div className="card">
      <div className="card-header">
        ...
        <div className="card-actions">
          <Button onClick={() => onEditModule(assignment)} variant="secondary" className="edit-btn">Edit Module</Button>
          <Button onClick={() => onDeleteModule(moduleId)} disabled={deleting[moduleId]} variant="danger">
            {deleting[moduleId] ? "Deleting..." : "Delete Module"}
          </Button>
          <Button onClick={onOpenStudentsModal} variant="primary">
      ...
      <div className="card-body">
        {subAssignments.length === 0 && assignmentPdf && (
          <div className="parent-pdf-view">
            <a href={assignmentPdf} target="_blank" rel="noopener noreferrer" className="pdf-link">
              View Assignment PDF
            </a>
          </div>
        )}
        ...
        {subAssignments.length > 0 && (
          <div style={{ overflowX: "auto" }}>
            <table className="table">
              <thead>
                <tr>
                  {["Sub-Module Name", "PDF", "Actions"].map(h => <th key={h} className="th">{h}</th>)}
                </tr>
              </thead>
              <tbody>
                    ...
                    <tr key={subId} className="tr">
                      ...
                      <td className="td">
                        <div className="action-buttons">
                          <Button onClick={() => onEditSubAssignment(assignment, sub)} variant="secondary" className="edit-sub-btn">
                            Edit
                          </Button>
                          <Button onClick={() => onDeleteSubAssignment(moduleId, subId)} disabled={deleting[subId]} variant="danger">
...
export default function AssignmentsManager() {
  ...
  // Edit handlers
  const handleEditModule = (assignment) => setEditingModule(assignment);
  const handleEditSubAssignment = (assignment, subAssignment) => setEditingSubAssignment({ assignment, subAssignment });
  const handleEditSave = () => { fetchAssignments(); setEditingModule(null); setEditingSubAssignment(null); };
  const handleEditCancel = () => { setEditingModule(null); setEditingSubAssignment(null); };

  // NEW: one-pop category flow
  const handleAddAssignmentClick = () => setShowCategoryModal(true);
  const handlePickCategory = (category) => {
    setShowCategoryModal(false);
    // Pass the chosen category to the add page
    navigate(`/assignment/add?category=${encodeURIComponent(category)}`);
    // Alternatively: navigate('/assignment/add', { state: { category } });
  };
  ...
  return (
    <div className="container">
      <div className="header-container">
        ...
        <Button onClick={handleAddAssignmentClick} variant="primary" className="add-assignment-btn">
          + Add Assignment
        </Button>
      </div>
      ...
      {assignments.length === 0 ? (
        <div className="no-assignments">
          ...
          <Button onClick={handleAddAssignmentClick} variant="primary">
      ...
      {editingSubAssignment && (
        ...
        >
          <EditAssignmentForm
            assignment={{ ...editingSubAssignment.assignment, subAssignments: [editingSubAssignment.subAssignment] }}
            students={students}
            onSave={handleEditSave}
            onCancel={handleEditCancel}
          />
